...
if opt and opt.entities and opt.entities ~= 'ALL' and opt.entities ~= 'Partition' then
  assert(rewtr.reltd_ents_wikiid_to_rltdid, 'Import relatedness.lua before ent_name_id.lua')
  rltd_only = true
end

if opt.entities == 'Partition' then
  assert(rewtr.reltd_ents_wikiid_to_rltdid, 'Import relatedness.lua before ent_name_id.lua')
end

-- Unk entity wikid:
unk_ent_wikiid = 1
local entity_wiki_txtfilename = nil
local entity_wiki_t7filename = nil
if opt.entities == 'Partition' then 
    entity_wiki_txtfilename = opt.root_data_dir .. 'basic_data/partition/wiki_name_id_map_part' .. opt.part_id .. '.txt'
    entity_wiki_t7filename = opt.root_data_dir .. 'generated/partition/ent_name_id_map' .. opt.part_id ..'.t7'
else
    entity_wiki_txtfilename = opt.root_data_dir .. 'basic_data/wiki_name_id_map.txt'
    entity_wiki_t7filename = opt.root_data_dir .. 'generated/ent_name_id_map.t7'
end
if rltd_only then
  entity_wiki_t7filename = opt.root_data_dir .. 'generated/ent_name_id_map_RLTD.t7'
end
RLTD_entity_wiki_t7filename = opt.root_data_dir .. 'generated/ent_name_id_map_RLTD.t7'
...
if paths.filep(entity_wiki_t7filename) then
  ...
  RLTD_e_id_name = torch.load(RLTD_entity_wiki_t7filename)
  ...
  for tmp_ent_wikiid, _ in pairs(rewtr.reltd_ents_wikiid_to_rltdid) do
