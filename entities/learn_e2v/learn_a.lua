...
function train_ent_vecs()
  ...
  elseif opt.entities == 'Partition' then
    num_batches_per_epoch = 4000
  end
  ...
  while true do
    ...
    if (epoch % save_every_num_epochs == 0) then
      if opt.entities == 'Partition' then
          torch.save(opt.root_data_dir .. 'generated/ent_vecs/Partition/part' .. opt.part_id .. '/ent_vecs__ep_' .. epoch .. '.t7', nn.Normalize(2):forward(lookup_ent_vecs.weight:float()))
          print('==> saving model to ' .. opt.root_data_dir .. 'generated/ent_vecs/Partition/part' .. opt.part_id .. '/ent_vecs__ep_' .. epoch .. '.t7')
      else
